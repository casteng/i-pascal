...
public class PsiUtil {
    ...
    public static <T extends PsiElement> Collection<T> findChildrenOfAnyType(@Nullable final PsiElement element,
        ...
        @NotNull final PsiElementProcessor<? super PsiElement> processor1 = new PsiElementProcessor<PsiElement>() {
            @Override
            public boolean execute(@NotNull final PsiElement each) {
                return processor.execute((T) each);
            }
        };

        PsiTreeUtil.processElements(element, processor1);
