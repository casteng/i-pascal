...
public class UsesActions {
    ...
    public static class SearchUnitAction extends BaseUsesAction {
        ...
        public void invoke(@NotNull Project project, Editor editor, PsiFile file) throws IncorrectOperationException {
            if (unitName != null) {
                PascalImportOptimizer.addUnitToSection(PsiUtil.getElementPasModule(file), Collections.singletonList(unitName), toInterface);
                EditorUtil.showInformationHint(editor, message("action.unit.search.added", unitName,
                        message(toInterface ? "unit.section.interface": "unit.section.implementation")));
            }
        }
        ...
        public String getText() {
            lazyInit();
            return unitName != null ? message("action.unit.search.found", unitName) : message("action.unit.search.notfound", namedElement.getName());
        }
